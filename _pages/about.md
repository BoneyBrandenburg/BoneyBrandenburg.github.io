---
permalink: /
title: "Ruiyang Gao - Ph.D. Student at Peking University"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

Welcome! I am **Ruiyang Gao**, a Ph.D. student in Computer Science at [Peking University](https://www.pku.edu.cn/), advised by [Prof. Daqing Zhang](https://www.cs.cmu.edu/~daqingz/) (IEEE Fellow).

## Research Interests

My research focuses on:

- **Mobile and Ubiquitous Computing**: Developing intelligent systems that sense and respond to human activities in everyday environments
- **Internet of Things (IoT)**: Building scalable IoT architectures for smart homes and cities
- **Wireless Sensing**: Leveraging WiFi signals and other wireless technologies for contactless sensing
- **Human-Computer Interaction**: Creating natural and intuitive interfaces between humans and computing systems

## About Me

I am currently pursuing my Ph.D. at the School of Computer Science, Peking University. My research explores the intersection of mobile computing, wireless sensing, and human-computer interaction. I have published multiple papers at top-tier venues including ACM UbiComp, IMWUT, and ACM SenSys, focusing on innovative sensing techniques using commercial WiFi devices.

## Contact

- **Email**: [railgun@gaoruiyang.cn](mailto:railgun@gaoruiyang.cn)
- **Google Scholar**: [Profile](https://scholar.google.com/citations?user=3utn_nYAAAAJ)
- **GitHub**: [BoneyBrandenburg](https://github.com/BoneyBrandenburg)
- **Institution**: Peking University, Beijing, China

Feel free to reach out if you're interested in my research or would like to collaborate!
